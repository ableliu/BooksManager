...
public class BookDetailActivity extends AppCompatActivity {
    ...
    private void lendOut() {
        ...
        mSure.setOnClickListener(v -> {
            ...
            BookDataSource.getInstance(mContext).lentOut(mBook.getId(), name, phone)
                    .flatMap(integer -> {
                        if (integer > 0) {
                            mCancel.setEnabled(true);
                            mSure.setEnabled(true);
                            ...
                            return BookDataSource.getInstance(mContext).getData(mBook.getBook_code());
                        } else {
                            mCancel.setEnabled(true);
                            mSure.setEnabled(true);
                            CustomToast.error(mContext, "借阅失败！");
                        }
                        return Observable.error(new Exception("入库失败！"));
                    })
    ...
    private void onReturn() {
        ...
        mSure.setOnClickListener(v -> {
            ...
            BookDataSource.getInstance(mContext).backBook(mBook.getId())
                    .flatMap(integer -> {
                        if (integer > 0) {
                            mCancel.setEnabled(true);
                            mSure.setEnabled(true);
